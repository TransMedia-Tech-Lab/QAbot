...
1. 依存関係をインストール
   ...
   uv sync
   ```
2. Ollama をインストール（ローカル LLM を使用する場合）
   - [Ollama 公式サイト](https://ollama.com/) から OS に応じたインストーラーをダウンロードしてインストール
   - モデルをダウンロード
     ```bash
     ollama pull llama3
     ```
   - 他の利用可能なモデルは `ollama list` で確認できます
3. 環境変数を設定
   ```bash
   cp .env.example .env
   # Slack / esa / LLM を記入
   ```
   - ローカル LLM を使用する場合は `.env` に `OLLAMA_MODEL=llama3` を設定
4. 初回起動時に自動で `chroma_db/` `logs/` が作成されます。必要に応じて事前に `mkdir -p chroma_db logs` しておいても構いません。
